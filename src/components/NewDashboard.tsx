              ...
              {reportData && (
                <div id="report-content" className="space-y-12">
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-800 text-lg leading-relaxed">
                      Derneğimiz tüm zamanlarda toplam <span className="font-bold text-blue-800">{formatCurrency(reportData.allTime.fin.tGelir)} TL</span> gelir hacmine erişmiştir. Bu dönemde toplam <span className="font-bold">{reportData.allTime.fin.eKSet.size}</span> kişiye <span className="font-bold">{formatCurrency(reportData.allTime.fin.egitim)} TL</span> eğitim yardımı, <span className="font-bold">{reportData.allTime.fin.sKSet.size}</span> kişiye <span className="font-bold">{formatCurrency(reportData.allTime.fin.sosyal)} TL</span> sosyal yardım ve diğer kalemlerde <span className="font-bold">{formatCurrency(reportData.allTime.fin.digerGider)} TL</span> olmak üzere <span className="font-bold text-rose-700">{formatCurrency(reportData.allTime.fin.tGider)} TL</span> giderde bulunmuştur.
                    </p>
                    <p className="text-slate-800 text-lg">
