            ...
            <div className="p-8 overflow-y-auto space-y-10 print:overflow-visible print:p-4">
              ...
              {reportData && (
                <div className="space-y-10">
                  ...
                  {/* 1. DÖNEMSEL ANALİZ */}
                  <div className="bg-blue-50/40 border-2 border-blue-100 p-8 rounded-3xl">
                    ...
                    <div className="space-y-4 text-slate-800 leading-relaxed italic text-base">
                      <p>Derneğimiz, <span className="font-bold">{reportDate.month}/{reportDate.year}</span> döneminde <span className="font-bold text-blue-700">{formatCurrency(reportData.monthly.fin.tGelir)} TL</span> toplam gelir elde etmiştir. Bu dönemde toplam <strong>{reportData.monthly.fin.kSet.size}</strong> kişiye <strong>{formatCurrency(reportData.monthly.fin.tGider)} TL</strong> yardım ulaştırılmıştır.</p>
                  ...
                  <div className="bg-emerald-50/40 border-2 border-emerald-100 p-8 rounded-3xl">
                    ...
                    <div className="space-y-4 text-slate-800 leading-relaxed italic text-base">
                      ...
                      <p className="not-italic font-bold bg-white/80 p-4 rounded-xl border border-emerald-200 text-emerald-800 shadow-sm text-sm">
                        Yıllık Dağılım: {reportData.yearly.fin.sKSet.size} kişi sosyal, {reportData.yearly.fin.eKSet.size} kişi eğitim yardımı almıştır. (Başarı Oranı: %{reportData.yearly.req.perc})
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="overflow-hidden border-2 border-slate-200 rounded-2xl shadow-sm">
                      <table className="w-full text-left border-collapse bg-white">
                        ...
                        <tbody className="text-sm text-slate-700 font-medium">
                          ...
                          <tr className="border-b"><td className="p-3 pl-6">Sosyal Yardım Alıcı Sayısı</td><td className="p-3 text-center font-bold">{reportData.monthly.fin.sKSet.size}</td><td className="p-3 text-center font-bold text-blue-600">{reportData.yearly.fin.sKSet.size}</td><td className="p-3 text-center font-bold text-blue-800">{reportData.allTime.fin.sKSet.size}</td></tr>
                          <tr className="border-b"><td className="p-3 pl-6">Eğitim Yardımı Alıcı Sayısı</td><td className="p-3 text-center font-bold">{reportData.monthly.fin.eKSet.size}</td><td className="p-3 text-center font-bold text-blue-600">{reportData.yearly.fin.eKSet.size}</td><td className="p-3 text-center font-bold text-blue-800">{reportData.allTime.fin.eKSet.size}</td></tr>
                          <tr className="border-b bg-blue-50 font-black text-blue-900"><td className="p-3 pl-6 uppercase text-[10px]">Toplam Farklı Kişi Sayısı</td><td className="p-3 text-center">{reportData.monthly.fin.kSet.size}</td><td className="p-3 text-center">{reportData.yearly.fin.kSet.size}</td><td className="p-3 text-center">{reportData.allTime.fin.kSet.size}</td></tr>

                          <tr className="border-b bg-slate-50/50"><td className="p-3 font-bold text-slate-900 text-xs" colSpan={4}>PERFORMANS GÖSTERGELERİ</td></tr>
                  ...
                  <div className="bg-amber-50 border-2 border-amber-100 p-6 rounded-2xl italic text-slate-800 text-sm">
                    <p><strong>Üyelik Durumu:</strong> Derneğimizin güncel aktif üye sayısı <span className="font-bold text-amber-900">{stats.activeMembers}</span>'dir. Rapor dönemi içerisinde yeni katılan üyelerimiz: {reportData.newMembers.length > 0 ? reportData.newMembers.join(', ') : 'Bulunmamaktadır'}.</p>
                  </div>
