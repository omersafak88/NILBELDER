...
export default function NewDashboard({ session, onLogout }: DashboardProps) {
  ...
  // Veri çekme fonksiyonu - Range mantığı iyileştirildi
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const processData = (data: any[]) => {
        const res = { 
          aidat: 0, bagis: 0, digerGelir: 0, 
          sosyalYardim: 0, egitimYardim: 0, digerYardim: 0, 
          sosyalKisiler: new Set<string>(), egitimKisiler: new Set<string>(),
          toplamKisiSet: new Set<string>() 
        };
        ...
        data.forEach(item => {
          ...
          if (item.member_id && memberMap.has(item.member_id)) {
            ...
            // Eğer kimlik belirlenemezse benzersiz bir ID ata ki sayım yapılabilsin
            person = `UNKNOWN-${Math.random()}`; 
          }
          ...
          if (item.type === 'income') {
            ...
            if (catName.includes('sosyal')) { 
              res.sosyalYardim += amt; 
              res.sosyalKisiler.add(person); 
              res.toplamKisiSet.add(person);
            }
            else if (catName.includes('eğitim') || catName.includes('egitim')) { 
              res.egitimYardim += amt; 
              res.egitimKisiler.add(person); 
              res.toplamKisiSet.add(person);
            }
            else {
              res.digerYardim += amt;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 relative pb-20 p-4">
      {/* İstatistik Kartları */}
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              <div className="flex gap-4">
                ...
                <button onClick={generateReport} disabled={reportLoading} className="bg-blue-600 text-white px-8 rounded-xl font-bold">
                   {reportLoading ? 'Veriler Çekiliyor...' : 'Raporu Oluştur'}
                </button>
              </div>
              ...
              {reportData && (
                <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                  ...
                  <div className="bg-blue-50 p-6 rounded-2xl text-slate-800 italic leading-relaxed border border-blue-100 shadow-sm">
                    <p className="font-semibold mb-2 not-italic text-blue-700 underline">Dönem Özeti ({reportDate.month}/{reportDate.year})</p>
                    ...
                    Ayrıca aynı dönemde <span className="font-bold text-blue-700 text-lg"> {reportData.monthly.toplamKisiSet.size} </span> farklı kişiye 
                    <span className="font-bold"> {formatCurrency(reportData.monthly.sosyalYardim + reportData.monthly.egitimYardim + reportData.monthly.digerYardim)} TL</span> yardımda bulundu."
                  </div>

                  {/* 2. YILLIK ÖZET */}
                  <div className="bg-emerald-50 p-6 rounded-2xl text-slate-800 italic leading-relaxed border border-emerald-100 shadow-sm">
                    <p className="font-semibold mb-2 not-italic text-emerald-700 underline">Yıllık Gelişim ({reportDate.year})</p>
                    ...
                    toplam <span className="font-bold text-emerald-700 text-lg"> {reportData.yearly.toplamKisiSet.size} </span> kişiye 
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl text-slate-800 italic leading-relaxed border border-slate-200 shadow-sm">
                    <p className="font-semibold mb-2 not-italic text-slate-700 underline">Kuruluştan Bugüne Genel Toplam</p>
                    ...
                    Bu süreçte toplam <span className="font-bold text-slate-900 text-lg"> {reportData.allTime.toplamKisiSet.size} </span> farklı ihtiyaç sahibine el uzatılmıştır."
                  ...
                  <div className="bg-amber-50 p-6 rounded-2xl text-slate-800 italic leading-relaxed border border-amber-100 shadow-sm">
                    <p className="font-semibold mb-2 not-italic text-amber-700 underline">Üyelik Bilgileri</p>
